...
func (r *HttpRequest) JsonBody(obj interface{}) *HttpRequest {
	...
	if err := json.NewEncoder(body).Encode(obj); nil != err {
		r.err = fmt.Errorf("Encode request body into JSON error: %v", err)
		return r
	}
...
// Do send request and return http.Response object and error.
// The http.Response.Body should be closed by caller
func (r *HttpRequest) DoRaw() (*http.Response, error) {
	...
	if nil != err {
		return nil, fmt.Errorf("Init request error: %v", err)
	}
	...
	if nil != err {
		return nil, fmt.Errorf("Request remote error: %v", err)
	}
